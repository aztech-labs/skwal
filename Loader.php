    ...
    class SplClassLoader
    {
        ...
        {           
            if ($this->shouldLoad($className)) {
                ...
                include_once ($this->includePath !== null ? $this->includePath . DIRECTORY_SEPARATOR : '') . $fileName;
            }
        }
        
        private function shouldLoad($className)
        {
            $classNamespace = substr($className, 0, strlen($this->namespace . $this->namespaceSeparator));
            
            return (null === $this->namespace || 
                $this->namespace . $this->namespaceSeparator === $classNamespace);
        }
    }
    
    $loader = new SplClassLoader('Skwal', __DIR__ . '/src');
